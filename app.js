...
(function () {
  ...
  async function processFile(file) {
    $('errBox').innerHTML = '';
    $('addMsg').textContent = '';
    if (!file.name.toLowerCase().endsWith('.xlsx')) { showErr('.xlsx 엑셀 파일만 업로드할 수 있습니다.'); return; }
    try {
      const zip = await JSZip.loadAsync(await file.arrayBuffer());
      const sheetPaths = Object.keys(zip.files).filter(p => /^xl\/worksheets\/sheet\d+\.xml$/.test(p)).sort();
      if (!sheetPaths.length) { showErr('엑셀에서 시트를 찾지 못했습니다.'); return; }
      ...
      [...doc.getElementsByTagName('row')].forEach(row => {
        ...
        if (bad && !isUsim) {
          hits.push({
            rn, svc: vals[SVC] || '', device: vals[DEVICE] || '', term: vals[TERM] || '',
  ...
  function renderCurrent(total, hits, fileName) {
    ...
    tb.innerHTML = hits.map(h =>
      '<tr><td>' + h.rn + '</td><td>' + cellOrBlank(h.svc) + '</td><td>' + cellOrBlank(h.device) + '</td><td>' + termCell(h.term) +
      '</td><td class="col-h">' + esc(h.h) + '</td><td>' + cellOrBlank(h.vals[0]) + '</td><td>' + cellOrBlank(h.vals[1]) +
      '</td><td>' + cellOrBlank(h.vals[2]) + '</td><td class="tag-hit">해당</td></tr>'
    ).join('');
  }
  ...
  function renderAcc() {
    ...
    tb.innerHTML = accumulated.map(a =>
      '<tr><td class="src">' + esc(a.file) + '</td><td>' + a.rn + '</td><td>' + cellOrBlank(a.svc) + '</td><td>' + cellOrBlank(a.device) +
      '</td><td>' + termCell(a.term) + '</td><td class="col-h">' + esc(a.h) + '</td><td>' + cellOrBlank(a.k) + '</td><td>' + cellOrBlank(a.n) +
      '</td><td>' + cellOrBlank(a.q) + '</td><td><button class="icon-btn" data-del data-file="' + escAttr(a.file) + '" data-rn="' + a.rn + '" title="삭제">' + xIcon() + '</button></td></tr>'
    ).join('');
  }
  ...
  function showErr(msg) { $('errBox').innerHTML = '<div class="err" style="margin-top:16px;color:var(--danger);font-size:14px;">' + esc(msg) + '</div>'; }
  ...
  function bind() {
    ...
    drop.ondrop = e => { e.preventDefault(); drop.classList.remove('is-drag'); if (e.dataTransfer.files[0]) processFile(e.dataTransfer.files[0]); };
    fileEl.onchange = e => { if (e.target.files[0]) { processFile(e.target.files[0]); fileEl.value = ''; } };

    // 업로드 데이터 추가 → 모음표
    $('btnAdd').onclick = () => {
      let added = 0;
      currentHits.forEach(h => {
        if (!accumulated.some(a => a.file === currentName && a.rn === h.rn)) {
          accumulated.push({ file: currentName, rn: h.rn, svc: h.svc, device: h.device, term: h.term, h: h.h, k: h.vals[0], n: h.vals[1], q: h.vals[2] });
          added++;
        }
      });
      save(); renderAcc();
